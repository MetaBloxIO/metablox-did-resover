...
export class EthrDidController {
   ...
   *
   * @param identifier - required - a `did:metablox` string or a publicKeyHex or an ethereum address
   ...
   *   did-metablox-registry contract v1.0.0 is used then this should be true.
   */
  constructor(
    ...
    this.did = publicKey ? `did:metablox:${networkString}${publicKey}` : `did:metablox:${networkString}${address}`
  }
  ...
  /**
   * The legacy version of the metablox-did-registry contract tracks the nonce as a property of the original owner, and not
