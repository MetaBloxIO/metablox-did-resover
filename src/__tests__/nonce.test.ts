...
describe('nonce tracking', () => {
  // let registry, accounts, did, identity, controller, delegate1, delegate2, metablox, didResolver
  ...
  beforeAll(async () => {
    ...
    legacyDid = `did:metablox:legacy:${identity}`
    did = `did:metablox:dev:${identity}`
  ...
  describe('new contract', () => {
    it('changing owner two times should result in original owner wallet nonce increase only once', async () => {
      ...
      const identifier = `did:metablox:dev:${originalOwner}`
      ...
      const metabloxController = new EthrDidController(
        identifier,
        registryContract,
      ...
      const hash = await metabloxController.createChangeOwnerHash(nextOwner)
      const signature = new SigningKey(originalOwnerPrivateKey).sign(hash)

      await metabloxController.changeOwnerSigned(nextOwner, {
        sigV: signature.v,
        sigR: signature.r,
        sigS: signature.s,
      })

      const hash2 = await metabloxController.createChangeOwnerHash(finalOwner)
      const signature2 = new SigningKey(nextOwnerPrivateKey).sign(hash2)

      await metabloxController.changeOwnerSigned(finalOwner, {
        sigV: signature2.v,
        sigR: signature2.r,
        sigS: signature2.s,
      })
    ...
    it('set attribute after owner change should result in original owner wallet nonce increase', async () => {
      ...
      const hash2 = await metabloxController.createSetAttributeHash(attributeName, attributeValue, attributeExpiration)
      const signature2 = new SigningKey(nextOwnerPrivateKey).sign(hash2)

      await metabloxController.setAttributeSigned(attributeName, attributeValue, attributeExpiration, {
        sigV: signature2.v,
        sigR: signature2.r,
        sigS: signature2.s,
      })
  ...
  describe('legacy contract', () => {
    it('changing owner two times should result in original owner wallet nonce increase', async () => {
      ...
      const identifier = `did:metablox:legacy:${originalOwner}`
