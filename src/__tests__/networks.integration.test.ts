...
describe('metabloxResolver (alt-chains)', () => {
  ...
  describe('eth-networks', () => {
    it('resolves on mainnet with versionId', async () => {
      ...
      const result = await resolver.resolve('did:metablox:0x26bf14321004e770e7a8b080b7a526d8eed8b388?versionId=12090174')
      expect(result).toEqual({
        ...
        didDocument: {
          ...
          id: 'did:metablox:0x26bf14321004e770e7a8b080b7a526d8eed8b388',
          verificationMethod: [
            {
              id: 'did:metablox:0x26bf14321004e770e7a8b080b7a526d8eed8b388#controller',
              type: 'EcdsaSecp256k1RecoveryMethod2020',
              controller: 'did:metablox:0x26bf14321004e770e7a8b080b7a526d8eed8b388',
              blockchainAccountId: 'eip155:1:0x26bF14321004e770E7A8b080b7a526d8eed8b388',
            },
          ],
          authentication: ['did:metablox:0x26bf14321004e770e7a8b080b7a526d8eed8b388#controller'],
          assertionMethod: ['did:metablox:0x26bf14321004e770e7a8b080b7a526d8eed8b388#controller'],
        },
      })
    })

    it('resolves on goerli when configured', async () => {
      const did = 'did:metablox:goerli:' + addr
    ...
    it('resolves on linea:goerli when configured', async () => {
      const did = 'did:metablox:linea:goerli:' + addr
      const metablox = getResolver({
    ...
    it('resolves on rsk when configured', async () => {
      const did = 'did:metablox:rsk:' + addr
      const metablox = getResolver({ networks: [{ name: 'rsk', rpcUrl: 'https://did.rsk.co:4444' }] })
      const resolver = new Resolver(metablox)
      const result = await resolver.resolve(did)
      expect(result).toEqual({
        didDocumentMetadata: {},
    ...
    it('resolves on rsk:testnet when configured', async () => {
      const did = 'did:metablox:rsk:testnet:' + addr
      const metablox = getResolver({ networks: [{ name: 'rsk:testnet', rpcUrl: 'https://did.testnet.rsk.co:4444' }] })
      const resolver = new Resolver(metablox)
      const result = await resolver.resolve(did)
      expect(result).toEqual({
        didDocumentMetadata: {},
    ...
    it('resolves public key identifier on rsk when configured', async () => {
      const did = 'did:metablox:rsk:0x03fdd57adec3d438ea237fe46b33ee1e016eda6b585c3e27ea66686c2ea5358479'
      const metablox = getResolver({ networks: [{ name: 'rsk', rpcUrl: 'https://did.rsk.co:4444' }] })
      const resolver = new Resolver(metablox)
