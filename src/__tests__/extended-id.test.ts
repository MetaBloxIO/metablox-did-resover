...
describe('interpretIdentifier', () => {
  ...
  it('parses did:metablox with address', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:metablox:${checksumAddress}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toBeUndefined()
    expect(network).toBeUndefined()
  })
  it('parses did:metablox with address and version', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:metablox:${checksumAddress}?versionId=42`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toBeUndefined()
    expect(network).toBeUndefined()
  })
  it('parses did:metablox with address and network', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:metablox:0x1:${checksumAddress}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toBeUndefined()
    expect(network).toEqual('0x1')
  })
  it('parses did:metablox with address and sub-network', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:metablox:rsk:testnet:${checksumAddress}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toBeUndefined()
    expect(network).toEqual('rsk:testnet')
  })
  it('parses did:metablox with address and sub-network and version', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:metablox:rsk:testnet:${checksumAddress}?versionId=42`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toBeUndefined()
    expect(network).toEqual('rsk:testnet')
  })
  ...
  it('parses did:metablox with publicKey', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:metablox:${pubKey}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toEqual(pubKey)
    expect(network).toBeUndefined()
  })
  it('parses did:metablox with publicKey and version', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:metablox:${pubKey}?versionId=42`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toEqual(pubKey)
    expect(network).toBeUndefined()
  })
  it('parses did:metablox with publicKey and network', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:metablox:mainnet:${pubKey}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toEqual(pubKey)
    expect(network).toEqual('mainnet')
  })
  it('parses did:metablox with publicKey and sub-network', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:metablox:not:so:main:net:${pubKey}`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toEqual(pubKey)
    expect(network).toEqual('not:so:main:net')
  })
  it('parses did:metablox with publicKey and sub-network', () => {
    const { address, publicKey, network } = interpretIdentifier(`did:metablox:not:so:main:net:${pubKey}?versionId=42`)
    expect(address).toEqual(checksumAddress)
    expect(publicKey).toEqual(pubKey)
    expect(network).toEqual('not:so:main:net')
  })
